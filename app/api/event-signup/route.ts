// Proxy for event signups. Streams the multipart form through to the
// platform API; enforces the 4 MB CV cap before forwarding (Vercel's own
// request ceiling is 4.5 MB). Mirrors app/api/apply/route.ts.
...
export async function POST(request: NextRequest) {
  ...
  if (typeof form.get('event') !== 'string') {
  ...
  try {
    const res = await fetch(`${upstream}/event-signup`, {
      method: 'POST',
      headers: {
